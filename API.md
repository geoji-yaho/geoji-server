...
> **한 사람은 방 3개까지만 참여할 수 있다(9/17).** 재판이 방마다 따로 돌아가므로 게시물 하나가
> 방 수만큼 AI 를 부른다 — 방 3개면 판결도 3건이고 토큰도 3배다. 상한을 넘기면 `409`.
> 삭제된 방은 세지 않으므로 방을 나가거나 지우면 자리가 다시 생긴다. 생성과 참가 모두에 적용된다.
...
**Response `409`** — 이미 방 3개에 들어가 있는 경우
```json
{ "message": "방은 최대 3개까지 참여할 수 있습니다. 쓰지 않는 방에서 나간 뒤 다시 시도해 주세요." }
```
...
**한 사람은 방 3개까지만** 참여할 수 있다. 넘기면 `409`. 이미 멤버인 방은 상한과 무관하게 통과한다.
...
{ "message": "유효하지 않은 초대 코드입니다." }
```
**Response `409`** — 이미 방 3개에 들어가 있는 경우(위와 같은 본문)
...
### `POST /api/rooms/{roomId}/ai-member`

'데모용 AI 유저 추가'. 방에 고정 AI 사용자 **떼거지봇**을 멤버로 넣고, 봇 명의의 템플릿 게시물 2개(`spent` 심야 택시
32,000원 · `considering` 무선 이어폰 189,000원)를 그 방에 올린다. 이후 이 방에 사람이 게시물을 올리면 5~10초 안에
떼거지봇이 방 강도에 맞는 사유로 투표한다(사람 1명 + 봇 방이면 봇 표로 바로 평결이 확정된다). **요청자가 그 방
멤버여야 한다.** 본문 없음. 봇은 방 참여 상한(3개)의 대상이 아니다.

멱등이다 — 이미 멤버이고 글 2개가 있으면 다시 만들지 않고 `200` 으로 같은 모양을 준다. 멤버 목록
`GET /api/rooms/{roomId}/members` 와 방 피드는 응답을 받은 뒤 다시 불러오면 된다.

**Response `201`** — 멤버 또는 글을 새로 만들었음 / **`200`** — 이미 다 있었음(같은 모양)
```json
{
  "userId": "7f3c9d2e-1a4b-4c8d-9e0f-123456789abc",
  "nickname": "떼거지봇",
  "postIds": ["3c9a1e7b-5d2f-4b8a-9e6c-1a2b3c4d5e6f", "0b8e5c1a-2f4d-4a6b-8c3e-9d1f2a7b6c50"]
}
```

| 필드 | 내용 |
|---|---|
| `userId` | 떼거지봇의 사용자 id. 멤버 목록에서 이 id 로 봇을 구분할 수 있다 |
| `postIds` | 템플릿 게시물 id. `spent`, `considering` 순 |

**오류**

| 상황 | 상태 코드 | 바디 |
|---|---|---|
| 방이 없거나 삭제됐거나 요청자가 멤버가 아님(방이 있는지 드러내지 않는다) | 404 | `{ "message": "방을 찾을 수 없습니다." }` |
| 서버에 떼거지봇 계정이 설정되지 않음 — 버튼을 숨기면 된다 | 503 | `{ "code": "AI_JUROR_NOT_CONFIGURED" }` |
| 템플릿 글 등록이 심문관 차단 등으로 실패 | 409 | `{ "message": "..." }` |
| JWT 없음 | 401 | - |
...
S-11 판결 공유 카드. 판결 문구와 이미지 metadata 만 준다.
금액·무엇을·사유·투표 사유·배심원·근거 원문·양형 이유는 없다.

- `room_id` 로 어느 방 판결의 카드인지 정한다. 강도는 그 방 강도다
- **AI 문구가 있으면 `headline`·`statement` 모두 AI 것을 그대로 준다**(9/19 결정).
  전에는 방 안에서만 볼 수 있는 근거를 인용한 문장을 템플릿으로 바꾸고 `headline` 까지 내렸는데,
  좋은 판결문일수록 방 규칙과 과거 지출을 인용해서 거의 모든 카드가 `유죄 / 배심원단이 …` 로만 나왔다.
  **카드를 방 밖으로 공유하면 그 안에 방 규칙이 함께 나갈 수 있다.** 그것을 감수한 선택이다
- AI 문구가 없거나 템플릿이면 템플릿 문구(`유죄`·`무죄`·`동의`·`기각`)다
- 삭제·공유 철회로 저장된 AI 문구를 쓸 수 없게 되면 전부 템플릿이다(이건 그대로다)
...
- **`room_id` 는 필수다.** 그 방 댓글만 준다 — **작성자라도 다른 방 댓글은 보지 않는다.**
  게시물은 여러 방에 올라가지만 댓글 스레드는 방마다 따로다
- 빠지거나 빈 값(`?room_id=`)이면 `400 { "message": "어느 방의 댓글인지 room_id 를 지정해 주세요." }`.
  빈 문자열은 Spring 이 null 로 바꿔 "안 준 것" 과 같아지므로 함께 막는다(9/17)
- 요청자가 그 방 멤버가 아니거나 공유가 철회된 방이면 `404`
